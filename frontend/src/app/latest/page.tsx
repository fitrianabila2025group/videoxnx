...
export const metadata = { title: 'Video Terbaru' };

export default async function Latest({ searchParams }: { searchParams: { page?: string } }) {
  const page = Number(searchParams.page || '1');
  let posts: Paginated<Post> = { data: [], page, per_page: 24, total: 0 };
  try { posts = await api(`/api/posts?per_page=24&page=${page}`); } catch {}
  return (
    <div className="container-x py-4 sm:py-6">
      ...
      <Pagination page={posts.page} perPage={posts.per_page} total={posts.total} basePath="/latest" />
    </div>
  );
}
